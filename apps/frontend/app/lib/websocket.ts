...
export class GroupsSocketClient {
  private socket: Socket | null = null;
  private handlers: Map<string, MessageHandler[]> = new Map();
  private url: string;
  private options: Record<string, unknown>;
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        this.socket = io(`${this.url}/groups`, { 
          transports: ['websocket'], 
          autoConnect: true,
          ...this.options
        });
        ...
        this.socket.on('connect', () => {
          console.log('[Groups Socket.IO] connected', this.socket?.id);
          resolve();
        });

        this.socket.on('disconnect', (reason: unknown) => {
          console.log('[Groups Socket.IO] disconnected', reason);
          this.emit('disconnect', { reason });
        });

        this.socket.on('connect_error', (error: Error) => {
          console.error('[Groups Socket.IO] connection error', error);
          this.emit('connection_error', { error: error.message });
        });

        this.socket.io.on('reconnect_attempt', (attempt: number) => {
          console.log('[Groups Socket.IO] reconnect attempt', attempt);
          this.emit('reconnect_attempt', { attempt });
        });

        this.socket.io.on('reconnect_failed', () => {
          console.error('[Groups Socket.IO] reconnection failed');
          this.emit('reconnect_failed', {});
        });
  ...
  send(event: string, payload: Record<string, unknown> = {}) {
    if (!this.socket) return console.warn('[Groups Socket.IO] not connected');
    console.log('[Groups Socket.IO] Sending event:', event, payload);
    this.socket.emit(event, payload);
  }
